...
public class RoutPoints extends GameCommand {
	...
	public void execute(final GameStatus gs, final List<String> commandArgs) {
		console.logNL(String.format("%s - rout points: %d",gs.getArmy1().getName(),gs.getArmy1().routPoints()));
		console.logNL(String.format("%s - rout points: %d",gs.getArmy2().getName(),gs.getArmy2().routPoints()));
	}
}
